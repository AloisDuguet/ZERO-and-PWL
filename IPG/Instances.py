...
def CyberSecurityGame(ins):
    ...
    for i in range(1,n_players+1):
        ...
        Q.append(read_matrix(ins+"/model_Q%i.csv"%i, n_var[i-1], n_var[i-1]))
        C.append(read_matrix(ins+"/model_C%i.csv"%i, sum(n_var[i] for i in range(n_players)) - n_var[i-1], n_var[i-1]))
    ...
    if n_players != 2:
        Raise("error not coded for more than two players")
    else:
        print(np.shape(Q[0]))
        print(np.shape(C[0]))
        print(np.shape(Q[1]))
        print(np.shape(C[1]))
        Q = [[Q[0],C[0]],[C[1],Q[1]]] # 8143.306158386399, 7297.113564998809 without transpositions and without constant terms
