...
for m in [3]:
    for n in [10,20,40]:
        f_knapsack = open("KP_table_3player.txt",'a')
        ...
        #for ins in [0]:
            ...
            # support size for 3 player game
            size_supp_ne = [sum(1 for s in ne[:len(S[0])] if s>10**-4),sum(1 for s in ne[len(S[0]):len(S[0])+len(S[1])] if s>10**-4),sum(1 for s in ne[len(S[0])+len(S[1]):] if s>10**-4)]
            ...
            if cpu_time>3600 or Profits_mSGM==[]:
                ...
                total_size_2 = total_size_1 + len(S[2])
                aux = aux +1
            ...
            if cpu_time_not_dfs>3600 or Profits_SGM==[]:
                ...
                total_size_2_not_dfs= total_size_1_not_dfs+ len(S[2])
        ...
        f_knapsack = open("KP_table_3player.txt",'a')
        f_knapsack.write(' & avg  & '+ "%0.2f" % total_time+' & '+"%0.2f" % total_iter+' & '+str(numb_pNE)+' & '+str(numb_mNE)+" & "+"%0.2f" % total_size_0+' & '+"%0.2f" % total_size_1+' & '+"%0.2f" % total_size_2+'  &')
        f_knapsack.close()
        ...
        total_size_2_not_dfs= (total_size_2_not_dfs*1.)/aux_not_dfs
        f_knapsack = open("KP_table_3player.txt",'a')
        f_knapsack.write("%0.2f" % total_time_not_dfs+' & '+"%0.2f" % total_iter_not_dfs+' & '+str(numb_pNE_not_dfs)+' & '+str(numb_mNE_not_dfs)+" & "+"%0.2f" % total_size_0_not_dfs+' & '+"%0.2f" % total_size_1_not_dfs+' & '+"%0.2f" % total_size_2_not_dfs+' \\\ \n')
        f_knapsack.close()
